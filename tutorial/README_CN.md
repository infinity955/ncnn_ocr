...
文字和坐标均与 PyTorch HF 完全一致。

---

## 支持的模型

一个 `ncnn_ocr` 可执行文件**同时支持多个 OCR 模型**，运行时按 `model.json` 的 `model_type` 自动分发
（`create_ocr()` 工厂 + `OCRBase` 抽象，见 [src/ocr_base.h](src/ocr_base.h)）：

| 模型 | model_type | 状态 | 运行 |
|------|-----------|------|------|
| **HunYuanOCR** | `hunyuan_ocr` | ✅ 跑通 | `--model .` |
| **GLM-OCR**（zai-org/GLM-OCR） | `glm_ocr` | ✅ 跑通，**自主转换** | `--model assets/glm_ocr` |

GLM-OCR 的 4 个 ncnn 模型由**本项目管线自己**从 PyTorch 转换（0 自定义层），逐模块对 PyTorch 验证通过，
端到端输出与标答一致，**未复用任何预转换权重**。全流程（下载→转换→编译→识别一致）与踩坑见
[docs/docs_glm/GLM-OCR-全流程总结.md](docs/docs_glm/GLM-OCR-全流程总结.md)。
...
CMake 工程在 `src/` 目录，源目录指向 `src/`；ncnn 自动探测 `../../ncnn/build/install/...`（相对 `src/`）。

```bash
# 在 ncnn_ocr/ 根目录
cmake -S src -B build
cmake --build build -j4
# （可选）覆盖 ncnn: cmake -S src -B build -Dncnn_DIR=$(realpath ../ncnn/build/install/lib/cmake/ncnn)
```

Windows (MSVC)：
```powershell
cd ..\ncnn_ocr
cmake -S src -B build -G "Visual Studio 17 2022" -A x64 -DUSE_OPENMP=OFF
...
```bash
# 在项目根目录 (ncnn_ocr/) 运行
./build/Release/ncnn_ocr --model . --image assets/testimg.jpg
```
...
```powershell
chcp 65001
.\build\Release\ncnn_ocr.exe --model . --image assets\testimg.jpg
```
...
> `verify_modules.py` 使用 `F.interpolate(pos_base, size=[gh, gw])` 插值 pos_embed，
> 与 HF 的 `scale_factor+0.1` 不同，因此 ViT maxabs ~2.8（见下表）。
> C++ 驱动使用 `scale_factor+0.1`（精确匹配 HF），故端到端输出完全一致。
...
### 坐标一致性：C++ 如何做到与 HF 逐字节一致

早期 C++ 输出**文字正确、但坐标与 HF 差 2–6 px**。根因在 ViT 的 position embedding 插值方式：
HF 用 `scale_factor=(h+0.1)/128` 做双线性插值；若图省事改用 `size=[gh,gw]`，每个位置会有约 0.163 的差异，
经 27 层 ViT 逐层放大到 maxabs ~2.82，最终把输出坐标带偏几个像素。

**解法**：把 pos_embed 从模型内部**外置**为 `pos_embed.bin`（72MB 基坐标），由 C++ 驱动在运行时按
每张图的网格、用与 HF **完全相同**的 `scale_factor=(h+0.1)/128` 双线性插值后再喂给 ViT。
ViT 精度随之从 2.82 降到 **0.0002**（等同 HF），于是：

- **文字逐字一致** —— 贪心解码每一步的 token 与 HF 完全相同；
- **坐标像素级一致** —— testimg.jpg 输出 `顺利上岸(231,392),(764,632)LAMAR(446,850),(559,878)05483(934,835),(997,870)`
  与 PyTorch 标答**逐字节相同**，2–6 px 偏差消失。
