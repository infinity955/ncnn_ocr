...
void BpeTokenizer::add_special_token_with_id(const std::string& token, int id) {
    // Add a special token with explicit ID to both the vocab and special tokens set
    token_to_id_[token] = id;
    id_to_token_[id] = token;
    additional_special_tokens_.push_back(token);
    additional_special_token_ids_.push_back(id);
    additional_special_token_to_id_[token] = id;
    additional_special_id_set_.insert(id);
}
...
std::string BpeTokenizer::decode(const std::vector<int>& ids, bool skip_special_tokens) const {
    ...
    for (int id : ids) {
        ...
        if (tok == "\\t")
            ...
            s += '\r';
        else if (!use_byte_encoder_ && tok.size() == 6 && tok[0] == '<' && tok[1] == '0' &&
                 tok[2] == 'x' && tok[5] == '>') {
            // SentencePiece byte_fallback token "<0xHH>" -> raw byte (fused into UTF-8 below)
            auto hx = [](char c) -> int {
            ...
            int hi = hx(tok[3]), lo = hx(tok[4]);
            if (hi >= 0 && lo >= 0) s += (char)((hi << 4) | lo);
            else s += tok;
        }
        else
            s += tok;
