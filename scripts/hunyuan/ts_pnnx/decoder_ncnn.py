...
def test_inference():
    torch.manual_seed(0)
    in0 = torch.rand(1, 8, 1024, dtype=torch.float)
    in1 = torch.rand(1, 1, 8, 8, dtype=torch.float)
    in2 = torch.rand(1, 8, 64, dtype=torch.float)
    in3 = torch.rand(1, 8, 64, dtype=torch.float)
    out = []
