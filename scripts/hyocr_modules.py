...
class VisionEncoder(nn.Module):
    def __init__(self, hidden: int = 1152, inter: int = 4304, num_layers: int = 27,
        ...
        # interpolatable 2D positional grid, stored as (1, hidden, pos_edge, pos_edge)
        self.register_buffer("pos_base", torch.zeros(1, hidden, pos_edge, pos_edge))
    ...
    def forward(self, pixels: torch.Tensor) -> torch.Tensor:
        ...
        gh = x.size(2)
        gw = x.size(3)
        # target size derived from input shape -> pnnx keeps this Interp dynamic
        # (scale_factor would bake to a float constant under trace). Small numeric
        # deviation from HF's (gh+0.1)/128 scale trick; validated end-to-end.
        pos = F.interpolate(self.pos_base, size=[gh, gw], mode="bilinear", align_corners=False)
        x = x + pos
